...
def calculate_gradients(img, sigma_d):
    ...
    dog_kernel_x = np.exp(-(x**2+y**2) / (2*sigma_d**2))
    dog_kernel_x = -x * dog_kernel_x / (2*np.pi*sigma_d**4)

    dog_kernel_y = np.exp(-(x**2+y**2) / (2*sigma_d**2))
    dog_kernel_y = -y * dog_kernel_y / (2*np.pi*sigma_d**4)
...
def calculate_structure_tensors(gx,gy,sigma_w):
    ...
    y = y - len(y)/2
    r = np.sqrt(x**2 + y**2)
    w = np.exp( (r-len(r)/2)**2 / (2*sigma_w**2) ) 
    w = w / (sigma_w*np.sqrt(2*np.pi))

    # sima = 2, r = (12,12), w=(12,12)
    
    S={}

    for x in range(len(gx)-len(w)):
        for y in range(len(gx[0])-len(w)):
...
def main():        
    ...
    img = cv2.cvtColor(input_image, cv2.COLOR_RGB2GRAY) # (205,286)
    
    sigma_d = 1
    gx,gy = calculate_gradients(img, sigma_d)    # (205,286), (205,286)
